...
interface TrailsContextType {
  upcomingTrails: Trail[];
  hikedTrails: Trail[];
  favorites: Set<string>;
  handleFavoriteClick: (e: React.MouseEvent, trailId: string) => void;
}

const Hiking = () => {
  const { t } = useTranslation();
  const { upcomingTrails } = useOutletContext<TrailsContextType>();
        ...
        {sortedTrails.length === 0 && (
          <div className={styles.noTrails}>
            <img
              ...
              alt={t("noUpcomingTrails")}
              className={styles.placeholderImage}
            />
            <p>{t("noUpcomingTrails")}</p>
          </div>
        )}
      </div>
    </div>
